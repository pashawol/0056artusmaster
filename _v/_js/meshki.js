...
function ItemCounter(){
    ...
    $('.js_item_not_available_block .js_counter').each(function(){
        ...
        $('.js_btn_counter_next', the_counter).click(function(e){
            ...
            interval = setTimeout(function() { $('.item_counter .info_text, .cart_items_block .item_counter .info_text').fadeOut(); }, 5000);
...
$(window).scroll(function(){
	...
	if(scrolled_top>192) {
		...
		$('.search_input_holder').css('margin-bottom', "145px");
	}
	else {
...
function SetCatalogMenuHeight(){
	...
	if($('.main_page_catalog_hidden_block').length>0){
		...
		var window_height = $(window).height();
		if(window_height<=catalog_height+300){
			$('.main_page_catalog_hidden_block').height(window_height-300);
			$('.js_scroll_pane').jScrollPane({							
				showArrows: true
			});
		}
...
$('.add_to_cart_block_1 .js_btn_add_to_cart_2').click(function(e){
        ...
        img.clone()
            .css({'position' : 'absolute', 'z-index' : '11100', 'width' : '500px', top: $(this).offset().top-430, left:$(this).offset().left-850})
            .appendTo("body")
